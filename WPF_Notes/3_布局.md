...
情景：保持用户界面独立部分的一致性；例如可以使独立的Grid控件具有相同的比例；

界面效果：

![image-20240121225857255](./assets/共享尺寸组.png)

+ 代码开发：

    + 注意事项;
        + 顶层Grid要配置Grid.IsSharedSizeScope="True"；
        + 在顶层Grid下的相同名称共享尺寸组才有效；
        + 也可以使用其它容器，如DockPanel、StackPanel；
    + 代码示例：
    ...
    <Grid Grid.IsSharedSizeScope="True">
        ...
        <Grid Grid.Row="0" Background="LightBlue" ShowGridLines="True" Margin="3">
            <Grid.ColumnDefinitions>
        ...
        <Grid Grid.Row="1" Margin="3" Background="LightGreen" ShowGridLines="True">
            <Grid.ColumnDefinitions>
                <ColumnDefinition Width="auto" SharedSizeGroup="TextLabel"/>
                <ColumnDefinition/>
            </Grid.ColumnDefinitions>
    ...
    + Canvas面板允许使用精确的坐标放置元素；如果设计数据驱动的富窗体和标准对话框，这并非好的选择；但如果构建其它一些不同的内容（如为图形工具构建绘图表面），canvas会比较合适；
    + Canvas中如何定位元素：设置Canvas.Left和Canvas.Top属性；

    ```xaml
        <Canvas ClipToBounds="True">
            <Button Canvas.Left="10" Canvas.Top="10">(10, 10)</Button>
            <Button Canvas.Left="50" Canvas.Top="70">(50, 70)</Button>
            <Button Canvas.Left="60" Canvas.Top="160">(60, 160)</Button>
        </Canvas>
    ```

    注意：为避免该面板下元素超出canvas面板，可设置ClipToBounds="True"；

    ### 6.2 Z顺序

+ 如果Canvas面板中有多个互相重叠的元素，可设置Canvas.Index属性控制上下顺序；
+ 默认情况下，Index值都为0，会按照代码前后顺序进行显示；（后声明的在上面）
+ 设置后，Index值大的会显示在上面；
